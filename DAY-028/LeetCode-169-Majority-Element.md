...
The majority element appears more than ⌊n/2⌋ times, which means it cannot be completely cancelled by all the other elements combined. The Boyer-Moore Voting Algorithm uses this observation by maintaining a candidate and a vote count. Matching elements increase the vote count, while different elements decrease it. Whenever the vote count becomes zero, a new candidate is selected. By the end of the traversal, the remaining candidate is guaranteed to be the majority element.

# Approach

1. Initialize `candidate` as `None` and `votes` as `0`.
2. Traverse the array once.
3. If `votes` is `0`, select the current element as the new candidate.
4. If the current element equals the candidate, increment `votes`.
5. Otherwise, decrement `votes`.
6. After the traversal, return the candidate.
